...
namespace VPKFILEPROCESSOR.FILEMANAGEMENTSERVICE.Tests
{
    public class AzureBlobStorageServiceTests
    {
        ...
        public async Task DeleteFileAsync_FileDoesNotExist_ThrowsException()
        {
            ...
            _mockBlobClient.Setup(blob => blob.ExistsAsync(It.IsAny<System.Threading.CancellationToken>())).ReturnsAsync(Response.FromValue(false, Mock.Of<Response>())); //Return false when ExistsAsync is called
        ...
        public async Task DownloadFileAsync_FileExists_ReturnsStream()
        {
            ...
            _mockBlobContainerClient.Setup(container => container.GetBlobClient(It.IsAny<string>())).Returns(_mockBlobClient.Object); //Return mockBlobClient when GetBlobClient is called
            _mockBlobClient.Setup(blob => blob.ExistsAsync(It.IsAny<System.Threading.CancellationToken>())).ReturnsAsync(Response.FromValue(true, Mock.Of<Response>())); //Return true when ExistsAsync is called
            _mockBlobClient.Setup(blob => blob.DownloadAsync(It.IsAny<System.Threading.CancellationToken>())).ReturnsAsync(mockDownloadResponse.Object); //Return mockDownloadResponse when DownloadAsync is called
